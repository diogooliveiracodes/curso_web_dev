<section>
    ...
    <form action="{{route('template.update')}}" 
          ...
          <select name="template" id="template"
                class='mt-6 border-gray-300 dark:border-gray-700 dark:text-gray-300 focus:border-indigo-300
                    focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm'>
            <option value="clean" @if ($user->blog->template == 'clean') selected  @endif>Clean</option>
            <option value="full"  @if ($user->blog->template == 'full') selected @endif>Full</option>
          </select>
